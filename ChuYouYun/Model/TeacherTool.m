...
//    NSString *path = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"teacher.sqlite"];
    NSString *path = [NSHomeDirectory() stringByAppendingPathComponent:@"Library/Private Documents/Temp/teacher.sqlite"];
...
+ (NSArray *)teacherWithDic:(NSMutableDictionary *)dic {
    ...
    //根据请求参数生成对应的SQL语句
    NSString *sql = @"SELECT * FROM t_teacher ORDER BY idstr DESC LIMIT 20";
    
    //执行SQL
    FMResultSet *set = [_db executeQuery:sql];
    NSMutableArray *classes = [NSMutableArray array];
    while (set.next) {
        NSData *blumData = [set objectForColumnName:@"teacher"];
...
+ (void)saveTeacheres:(NSArray *)SYGTeacher {
    ...
    for (NSDictionary *class in SYGTeacher ) {
        ...
        [_db executeUpdateWithFormat:@"INSERT INTO t_teacher(teacher ,idstr) VALUES (%@,%@);",classData,class];
    }
