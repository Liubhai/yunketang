...
#import "GSMJRefreshBackFooter.h"

@interface GSMJRefreshBackFooter()
@property (assign, nonatomic) NSInteger lastRefreshCount;
@property (assign, nonatomic) CGFloat lastBottomDelta;
@end

@implementation GSMJRefreshBackFooter
...
- (void)scrollViewContentOffsetDidChange:(NSDictionary *)change
{
    ...
    // 如果正在刷新，直接返回
    if (self.state == GSMJRefreshStateRefreshing) return;
    ...
    if (self.state == GSMJRefreshStateNoMoreData) {
    ...
    if (self.scrollView.isDragging) {
        ...
        if (self.state == GSMJRefreshStateIdle && currentOffsetY > normal2pullingOffsetY) {
            // 转为即将刷新状态
            self.state = GSMJRefreshStatePulling;
        } else if (self.state == GSMJRefreshStatePulling && currentOffsetY <= normal2pullingOffsetY) {
            // 转为普通状态
...
- (void)setState:(GSMJRefreshState)state
{
    ...
    if (state == GSMJRefreshStateNoMoreData || state == GSMJRefreshStateIdle) {
        // 刷新完毕
        if (GSMJRefreshStateRefreshing == oldState) {
        ...
        // 刚刷新完毕
        if (GSMJRefreshStateRefreshing == oldState && deltaH > 0 && currentCount != self.lastRefreshCount) {
