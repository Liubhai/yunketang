//
//  Good_LibraryScreenViewController.m
//  YunKeTang
//
...
@interface Good_LibraryScreenViewController (){
    CGFloat rankButtonX;
    NSInteger Number;
    BOOL isOne;
...
@implementation Good_LibraryScreenViewController
...
- (void)interFace {
    ...
    //添加通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(removeSelfView) name:@"Good_LibraryScreenRemove" object:nil];
}
...
#pragma mark --- 通知
-(void)removeSelfView {
    [[NSNotificationCenter defaultCenter] postNotificationName:@"NSNotificationGood_LibraryScreenButton" object:nil];
...
- (void)sureButtonCilck:(UIButton *)button {
    if (button.tag == 0) {//重置
        ...
        //发送通知
        [[NSNotificationCenter defaultCenter] postNotificationName:@"NSNotificationGood_LibraryScreenTypeID" object:dict];
        [self removeSelfView];
    }
}
