...
- (void)interFace {
    ...
    //添加通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(removeSelfView) name:@"TestClassMoreRemove" object:nil];
}

- (void)addScrollView {
    
    _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, MainScreenWidth,80)];
...
- (void)addTypeView {
    ...
    NSArray *titleArray = @[@"简单",@"普通",@"困难"];
    _typeArray = @[@"1",@"2",@"3"];
    
//    CGFloat buttonW = (MainScreenWidth / 2 - 10 - 7.5 - 15) / 2;
    CGFloat buttonW = (MainScreenWidth - 60) / 3;
    CGFloat buttonH = 30;
    //添加按钮
...
#pragma mark --- 通知
-(void)removeSelfView {
    [[NSNotificationCenter defaultCenter] postNotificationName:@"NSNotificationTestMoreButton" object:nil];
...
- (void)typeButtonCilck:(UIButton *)button {
    ...
    //发送通知
    [[NSNotificationCenter defaultCenter] postNotificationName:@"NSNotificationTestMoreTypeID" object:_typeStr];
    [self removeSelfView];
}
...
//        [self addButtonView];
        ...
        //发送通知
        [[NSNotificationCenter defaultCenter] postNotificationName:@"NSNotificationTestMoreTypeID" object:dict];
        [self removeSelfView];
    }
}
