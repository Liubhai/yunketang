...
- (UILabel *)fullScreenTimeLabel{
    if (!_fullScreenTimeLabel) {
        ...
        NSString *curTimeStr = @"00:00";
        NSString *totalTimeStr = @"00:00";
...
- (void)layoutSubviews{
    ...
    self.fullScreenButton.frame = CGRectMake(width - ALYPVBottomViewFullScreenButtonWidth - 10, 0 - 5,ALYPVBottomViewFullScreenButtonWidth + 10, height + 10);//这行改过
...
//                                             height);
        self.progressView.frame = CGRectMake(ALYPVBottomViewPlayButtonWidth + 20, 0,
                                             width - (ALYPVBottomViewPlayButtonWidth + ALYPVBottomViewFullScreenButtonWidth) - 40,
                                             height);
        self.progressView.frame = CGRectMake(ALYPVBottomViewPlayButtonWidth + 5, 0,
                                             width - (ALYPVBottomViewPlayButtonWidth + ALYPVBottomViewFullScreenButtonWidth) - 60,
                                             height);
        
        CGRect progressFrame = self.progressView.frame;
//        self.leftTimeLabel.frame = CGRectMake(progressFrame.origin.x, height/2, progressFrame.size.width/2, height/2);
        self.leftTimeLabel.frame = CGRectMake(10, height/4, progressFrame.size.width/2, height/2);
//        self.rightTimeLabel.frame = CGRectMake(progressFrame.origin.x + progressFrame.size.width/2,height/2, progressFrame.size.width/2, height/2);
        
        self.rightTimeLabel.frame = CGRectMake(progressFrame.origin.x + progressFrame.size.width,height/4, progressFrame.size.width/4, height/2);
...
//        self.qualityButton.selected = NO;
        ...
        self.fullScreenTimeLabel.frame = CGRectMake(ALYPVBottomViewPlayButtonWidth / 2, 0,ALYPVBottonViewFullScreenTimeWidth, height);
        ...
        self.progressView.frame = CGRectMake(ALYPVBottomViewPlayButtonWidth / 2 + ALYPVBottonViewFullScreenTimeWidth + ALYPVBottomViewMargin, 0, width - (ALYPVBottomViewPlayButtonWidth + ALYPVBottonViewFullScreenTimeWidth + 2 * ALYPVBottomViewMargin  + qualityWidth + ALYPVBottomViewFullScreenButtonWidth), height);
    }
}
...
//    [self.fullScreenButton setBackgroundImage:[AliyunUtil imageWithNameInBundle:@"al_play_screen_full" skin:skin] forState:UIControlStateNormal];
    [self.fullScreenButton setImage:[UIImage imageNamed:@"alivc_fullScreen_set@2x"] forState:UIControlStateSelected];
...
- (void)updateProgressWithCurrentTime:(float)currentTime durationTime:(float)durationTime{
    ...
    //时间的优化
    NSString *totalStr = [totalTimeStr substringToIndex:2];
    if ([totalStr isEqualToString:@"00"]) {//说明没有小时
        self.rightTimeLabel.text = [totalTimeStr substringFromIndex:3];
    }
    
    NSString *currentStr = [curTimeStr substringToIndex:2];
    if ([currentStr isEqualToString:@"00"]) {//说明没有小时
        self.leftTimeLabel.text = [curTimeStr substringFromIndex:3];
    }
    ...
    NSString *curTimeLast = [curTimeStr substringFromIndex:3];
    NSString *totalTimeLast = [totalTimeStr substringFromIndex:3];
    
    if ([totalStr isEqualToString:@"00"]) {//说明没有小时
        time = [NSString stringWithFormat:@"%@/%@", [curTimeStr substringFromIndex:3], [totalTimeStr substringFromIndex:3]];
        
        
        str = [[NSMutableAttributedString alloc] initWithString:time];
        [str addAttribute:NSForegroundColorAttributeName value:kALYPVColorTextNomal range:NSMakeRange(0, curTimeLast.length)];
        [str addAttribute:NSForegroundColorAttributeName value:kALYPVColorTextGray range:NSMakeRange(curTimeLast.length, curTimeLast.length + 1)];
        self.fullScreenTimeLabel.attributedText = str;
        
    } else if ([currentStr isEqualToString:@"00"]) {//说明没有小时
        time = [NSString stringWithFormat:@"%@/%@", currentStr, totalTimeStr];
    } else {
