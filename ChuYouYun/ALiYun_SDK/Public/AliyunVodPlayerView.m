...
- (UIButton *)playButton{
    if (!_playButton) {
        ...
        _playButton.center = CGPointMake(self.center.x, self.center.y - 20);
        [_playButton setBackgroundImage:[AliyunUtil imageWithNameInBundle:@"al_play_stop"] forState:UIControlStateNormal];
        [_playButton setBackgroundImage:[AliyunUtil imageWithNameInBundle:@"al_play_start@3x"] forState:UIControlStateSelected];
//        [_playButton setBackgroundImage:Image(@"al_play_start@2x") forState:UIControlStateSelected];
        [_playButton addTarget:self action:@selector(playButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
        _playButton.backgroundColor = [UIColor colorWithWhite:0.2 alpha:0.5];
    }
    return _playButton;
}
...
- (void)initView{
    ...
    //添加通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(getShow:) name:@"PlayButtonShow" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(getHidden:) name:@"PlayButtonHidden" object:nil];
}
...
- (void)getShow:(NSNotification *)not {
    self.playButton.hidden = NO;
}

- (void)getHidden:(NSNotification *)not {
     self.playButton.hidden = YES;
}
...
//            [self.popLayer showPopViewWithCode:ALYPVPlayerPopCodePlayFinish popMsg:nil];
            ...
            //自己添加 （播放完成后的操作）
            [[NSNotificationCenter defaultCenter] postNotificationName:@"AilYunPlayerPlayEnd" object:@"100"];
        }
            break;
