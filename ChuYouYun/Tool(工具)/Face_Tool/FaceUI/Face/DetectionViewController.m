...
@property (strong ,nonatomic)NSString           *goodStr;
        ...
        switch (remindCode) {
            case DetectRemindCodeOK: {
                ...
                if (images[@"bestImage"] != nil && [images[@"bestImage"] count] != 0) {
                    ...
                    _goodStr = @"good";
                    [[NSNotificationCenter defaultCenter] postNotificationName:@"NSNotficaitonFaceType" object:_goodStr];
                }
...
- (void)closeAction {
    ...
    if ([_formStr integerValue] == 1) {//注册
        if (_goodStr == nil) {
            [self.navigationController popToViewController:[self.navigationController.viewControllers objectAtIndex:3] animated:YES];
        } else {//说明已经照好
            [self.navigationController popViewControllerAnimated:YES];
        }
    } else if ([_formStr integerValue] == 2) {//考试
        if (_goodStr == nil) {
            [self.navigationController popToViewController:[self.navigationController.viewControllers objectAtIndex:2] animated:YES];
        } else {//说明已经照好
            [self.navigationController popViewControllerAnimated:YES];
        }
    } else if ([_formStr integerValue] == 3) {//课程
